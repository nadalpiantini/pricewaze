...
export async function generateContractDraft(input: ContractInput): Promise<ContractDraft> {
  const prompt = `You are a legal document assistant specializing in Dominican Republic real estate. Generate a DRAFT purchase agreement (Contrato de Compraventa) based on the following information.
...
- Agreed Price: $${input.agreedPrice.toLocaleString()} USD
...
export async function generateOfferLetter(
  ...
  const prompt = `Generate a brief, professional offer letter in Spanish for a real estate offer in the Dominican Republic.

Buyer: ${buyer.full_name}
Seller: ${seller.full_name}
Property: ${property.title} at ${property.address}
Asking Price: $${property.price.toLocaleString()} USD
Offer Amount: $${offerAmount.toLocaleString()} USD
...
Después de considerar cuidadosamente el valor de la propiedad y las condiciones actuales del mercado, me permito presentar una oferta formal de $${offerAmount.toLocaleString()} USD.
